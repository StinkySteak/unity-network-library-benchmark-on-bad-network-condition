...
public class EnhancedNetworkDebugStartGUI : Fusion.Behaviour
{
    ...
    EnhancedNetworkDebugStart _networkDebugStart;
    string _clientCount;
    bool _isMultiplePeerMode;

    Dictionary<EnhancedNetworkDebugStart.Stage, string> _nicifiedStageNames;
    ...
    protected virtual void Awake()
    {
        ...
        _nicifiedStageNames = ConvertEnumToNicifiedNameLookup<EnhancedNetworkDebugStart.Stage>("Fusion Status: ");
        _networkDebugStart = EnsureNetworkDebugStartExists();
        _clientCount = _networkDebugStart.AutoClients.ToString();
        ValidateClientCount();
    }
    ...
    protected EnhancedNetworkDebugStart EnsureNetworkDebugStartExists()
    {
        ...
        if (TryGetBehaviour<EnhancedNetworkDebugStart>(out var found))
        {
            _networkDebugStart = found;
            return found;
        }

        _networkDebugStart = AddBehaviour<EnhancedNetworkDebugStart>();
        return _networkDebugStart;
    }
    ...
    private void Update()
    {
        ...
        if (nds.StartMode != EnhancedNetworkDebugStart.StartModes.UserInterface)
        {
            return;
        }

        var currentstage = nds.CurrentStage;
        if (currentstage != EnhancedNetworkDebugStart.Stage.Disconnected)
        {
            return;
        }
    ...
    protected virtual void OnGUI()
    {
        ...
        var currentstage = nds.CurrentStage;
        if (nds.AutoHideGUI && currentstage == EnhancedNetworkDebugStart.Stage.AllConnected)
        {
            return;
        }
        ...
        GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
        {
            GUILayout.BeginVertical(GUI.skin.window);
            {
                GUILayout.BeginHorizontal(GUILayout.Height(height));
                {
                    ...
                    if (nds.AutoHideGUI == false && nds.CurrentStage == EnhancedNetworkDebugStart.Stage.AllConnected)
                    {
    ...
    private void StartHostWithClients(EnhancedNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartServerWithClients(EnhancedNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartMultipleClients(EnhancedNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartMultipleAutoClients(EnhancedNetworkDebugStart nds)
    {
        int.TryParse(_clientCount, out int count);
        nds.StartMultipleAutoClients(count);
    }

    private void StartMultipleSharedClients(EnhancedNetworkDebugStart nds)
    {
        int count;
        try
        {
