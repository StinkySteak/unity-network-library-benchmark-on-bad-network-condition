...
using System.Linq;
using StinkySteak.NetcodeLatency.FusionNet;
...
public class EnhancedNetworkDebugStart : Fusion.Behaviour
{
    ...
    public event Action<NetworkRunner> OnStartingGame;

#if UNITY_EDITOR
    protected virtual void Reset()
    {
        if (TryGetComponent<EnhancedNetworkDebugStart>(out var ndsg) == false)
        {
            ndsg = gameObject.AddComponent<EnhancedNetworkDebugStart>();
        }
    }
    ...
    protected virtual void Start()
    {
        ...
        if (existingrunner && existingrunner != RunnerPrefab)
        {
            if (existingrunner.State != NetworkRunner.States.Shutdown)
            {
                ...
                var gui = GetComponent<EnhancedNetworkDebugStartGUI>();
                if (gui)
                {
                    Destroy(gui);
                }
        ...
        {
            if (TryGetComponent<EnhancedNetworkDebugStart>(out var _) == false)
            {
                gameObject.AddComponent<EnhancedNetworkDebugStart>();
            }
        }
    }
    ...
    protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
    {
        ...
        if (includesServerStart)
        {
            ...
            var serverTask = InitializeNetworkRunner(_server, serverMode, NetAddress.Any(ServerPort), sceneRef, null);
...
#else
        var netAddress = NetAddress.Any();

        if (mode == GameMode.AutoHostOrClient)
            NetAddress.Any(7777);

        var clientTask = InitializeNetworkRunner(client, mode, netAddress, sceneRef, null);
#endif
