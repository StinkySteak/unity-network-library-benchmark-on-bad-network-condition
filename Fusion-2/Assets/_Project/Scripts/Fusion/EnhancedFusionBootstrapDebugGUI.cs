namespace Fusion
{
    ...
    /// <summary>
    /// Companion component for <see cref="EnhancedFusionBootstrap"/>. Automatically added as needed for rendering in-game networking IMGUI.
    /// </summary>
    [ScriptHelp(BackColor = ScriptHeaderBackColor.Steel)]
    public class EnhancedFusionBootstrapDebugGUI : Fusion.Behaviour
    {
        ...
        EnhancedFusionBootstrap _networkDebugStart;
        string _clientCount;
        bool _isMultiplePeerMode;

        Dictionary<EnhancedFusionBootstrap.Stage, string> _nicifiedStageNames;
        ...
        protected virtual void Awake()
        {
            ...
            _nicifiedStageNames = ConvertEnumToNicifiedNameLookup<EnhancedFusionBootstrap.Stage>("Fusion Status: ");
            _networkDebugStart = EnsureNetworkDebugStartExists();
            _clientCount = _networkDebugStart.AutoClients.ToString();
            ValidateClientCount();
        }
        ...
        protected EnhancedFusionBootstrap EnsureNetworkDebugStartExists()
        {
            ...
            if (TryGetBehaviour<EnhancedFusionBootstrap>(out var found))
            {
                _networkDebugStart = found;
                return found;
            }

            _networkDebugStart = AddBehaviour<EnhancedFusionBootstrap>();
            return _networkDebugStart;
        }
        ...
        private void Update()
        {
            ...
            if (currentstage != EnhancedFusionBootstrap.Stage.Disconnected)
            {
                return;
            }
        ...
        protected virtual void OnGUI()
        {
            ...
            var currentstage = nds.CurrentStage;
            if (nds.AutoHideGUI && currentstage == EnhancedFusionBootstrap.Stage.AllConnected)
            {
                return;
            }
            ...
            GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
            {
                GUILayout.BeginVertical(GUI.skin.window);
                {
                    GUILayout.BeginHorizontal(GUILayout.Height(height));
                    {
                        ...
                        if (nds.AutoHideGUI == false && nds.CurrentStage == EnhancedFusionBootstrap.Stage.AllConnected)
                        {
        ...
        private void StartHostWithClients(EnhancedFusionBootstrap nds)
        {
            int count;
            try
            {
        ...
        private void StartServerWithClients(EnhancedFusionBootstrap nds)
        {
            int count;
            try
            {
        ...
        private void StartMultipleClients(EnhancedFusionBootstrap nds)
        {
            int count;
            try
            {
        ...
        private void StartMultipleAutoClients(EnhancedFusionBootstrap nds)
        {
            int.TryParse(_clientCount, out int count);
            nds.StartMultipleAutoClients(count);
        }

        private void StartMultipleSharedClients(EnhancedFusionBootstrap nds)
        {
            int count;
            try
            {
