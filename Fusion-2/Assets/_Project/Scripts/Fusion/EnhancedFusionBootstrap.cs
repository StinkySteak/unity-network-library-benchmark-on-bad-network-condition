namespace Fusion
{
    ...
    public class EnhancedFusionBootstrap : Fusion.Behaviour
    {

        public event Action<NetworkRunner> OnStart;

        /// <summary>
        /// Selection for how <see cref="EnhancedFusionBootstrap"/> will behave at startup.
        /// </summary>
        ...
        /// <summary>
        /// <see cref="EnhancedFusionBootstrapDebugGUI"/> will not render GUI elements while <see cref="CurrentStage"/> == <see cref="Stage.AllConnected"/>.
        /// </summary>
        [InlineHelp]
        ...
        /// If this field is null or invalid, will be set to the current scene when <see cref="EnhancedFusionBootstrap"/> runs Awake().
        /// </summary>
        [InlineHelp]
        [ScenePath]
        ...
        /// <summary>
        /// Indicates which step of the startup process <see cref="EnhancedFusionBootstrap"/> is currently in.
        /// </summary>
        [InlineHelp]
        [SerializeField]
        [ReadOnly]
        ...
        /// <summary>
        /// Requires Multiplayer Play Mode (MPPM) to be installed. If enabled, <see cref="EnhancedFusionBootstrap"/> will automatically join the virtual instance.
        /// </summary>
        ...
        {
            if (TryGetComponent<EnhancedFusionBootstrapDebugGUI>(out var ndsg) == false)
            {
                ndsg = gameObject.AddComponent<EnhancedFusionBootstrapDebugGUI>();
            }
        }
        ...
        protected virtual void Start()
        {
            ...
            if (existingRunner && existingRunner != RunnerPrefab)
            {
                if (existingRunner.State != NetworkRunner.States.Shutdown)
                {
                    ...
                    var gui = GetComponent<EnhancedFusionBootstrapDebugGUI>();
                    if (gui)
                    {
                        Destroy(gui);
                    }
        ...
        {
            if (TryGetComponent<EnhancedFusionBootstrapDebugGUI>(out var gui) == false)
            {
                gui = gameObject.AddComponent<EnhancedFusionBootstrapDebugGUI>();
            }
            gui.enabled = true;
        }
        ...
        protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
        {
            ...
            {
                Debug.LogWarning($"{nameof(EnhancedFusionBootstrap)} can't be a child game object, un-parenting.");
